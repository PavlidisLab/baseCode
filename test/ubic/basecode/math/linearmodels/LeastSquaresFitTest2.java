...
public class LeastSquaresFitTest2 {
    ...
    public void testMatrixWeightedMeanVariance() throws Exception {
         ...
         */
        assertArrayEquals( new double[] { 10.99327, 10.99189, 10.04471, 10.17934, 10.56090, 10.48717, 10.48942, 10.64346, 10.50560, 10.55924 },
            est.getNormalizedValue().viewRow( 0 ).toArray(),
                0.00001);
        ...
        assertNotNull( anova );
        assertEquals( 7, ( int ) anova.getResidualsDof() );
        ...
        assertEquals(0.6174535, anova.getMainEffectFStat("genotype"), 0.001); // F=1.4354/2.3248
        assertEquals(0.5663, anova.getMainEffectPValue("genotype"), 0.0001);

        assertEquals( 7, ( int ) f1.getResidualsDof() ); // ok

        assertEquals(0.15, f1.getRSquared(), 0.01);
        assertEquals(-0.09291, f1.getAdjRSquared(), 0.0001);
        assertEquals(0.6174535, f1.getFStat(), 0.0001);
        assertEquals(0.5663, f1.getOverallPValue(), 0.0001);
...
// same as above, checking my sanity and checking that running summarize more than once doesn't mess anything up.
        ...
        assertNotNull( reA );
        assertEquals(0.6174535, reA.getMainEffectFStat("genotype"), 1e-4);
        ...
        assertNotNull( reE );
        assertEquals(0.73359941990489119235, reE.getMainEffectFStat("genotype"), 1e-2);
