...
public class JMatrixDisplay extends JPanel {
	...
	// data fields
	ColorMatrix colorMatrix; // reference to standardized or unstandardized
								// matrix
	...
	protected Dimension getSize(boolean withLabels) {
		...
		m_rowLabelWidth = m_labelGutter
				+ Util.maxStringPixelWidth(colorMatrix.getRowNames(),
						m_labelFont, this);
		...
		if (m_maxColumnLength > 0) {
			...
			// fix column height to ~5 pixels per character. This prevents a
			// slightly different column height
			// for different letters.
			m_columnLabelHeight = 5 * m_maxColumnLength;
		} else {
			m_columnLabelHeight = Util.maxStringPixelWidth(colorMatrix
					.getColumnNames(), m_labelFont, this);
		}

		// m_columnLabelHeight += m_labelGutter; // this is optional (leaves
		// some
		// space on top)
	 ...
	 * @param g
	 *            Graphics
	 * @param leaveRoomForLabels
	 *            boolean
	 */
	protected void drawMatrix(ColorMatrix matrix, Graphics g,
	...
	protected void drawRowNames(Graphics g) {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			int y = (i * m_cellSize.height) + m_columnLabelHeight
					+ m_labelGutter;
			int xRatio = (colorMatrix.getColumnCount() * m_cellSize.width)
					+ m_labelGutter;
	...
	/**
	 * Pads a string to the maxColumnLength. If it is over the maxColumnLength,
	 * it abbreviates it to the maxColumnLength
	 * 
	 * @param str
	 * @return
	 */
	...
	private void setFont() {
		int fontSize = Math.min(getFontSize(), (int) ((double) m_maxFontSize
	...
	public void saveImage(String outPngFilename) throws java.io.IOException {
		saveImage(this.colorMatrix, outPngFilename, m_isShowLabels,
				m_isShowingStandardizedMatrix);
	}

	public void saveImage(String outPngFilename, boolean showLabels)
			throws java.io.IOException {
		saveImage(this.colorMatrix, outPngFilename, showLabels,
				m_isShowingStandardizedMatrix);
	}
	 ...
	 *            boolean
	 * @param standardize
	 *            normalize to deviation 1, mean 0.
	 * @todo never read
	 * @throws IOException
	 */
	public void saveImage(ColorMatrix matrix, String outPngFilename,
			boolean showLabels, boolean standardize) throws java.io.IOException {
		...
		// column labels
		BufferedImage m_image = new BufferedImage(d.width, d.height,
	 ...
	 * @param standardize
	 */
	public void saveImageToPng(ColorMatrix matrix, OutputStream stream,
			boolean showLabels, boolean standardize) {
	...
	public void writeToPng(ColorMatrix matrix, OutputStream stream,
			boolean showLabels) {
		// Draw the image to a buffer
		Dimension d = getSize(showLabels); // how big is the image with row and
		// column labels
		...
		try {
			...
			log.error("Error writing image to output stream.  Stacktrace is: "
					+ e.toString());
		}
	}
	...
	/**
	 * @param matrix
	 *            the new matrix to use; will resize this display component as
	 *            necessary
	 */
	...
	/**
	 * @param colorMap
	 *            an array of colors which define the midpoints in the color
