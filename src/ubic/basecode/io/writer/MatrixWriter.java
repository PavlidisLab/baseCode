    ...
    public void writeMatrix( AbstractNamedMatrix matrix, boolean printNames ) throws IOException {
        ...
        if ( printNames ) {
            for ( Iterator it = matrix.getColNames().iterator(); it.hasNext(); ) {
                ...
                buf.append( sep );
                if ( colNameMap != null && colNameMap.get( colName ) != null ) colName = colNameMap.get( colName );
                buf.append( colName );
            }
            buf.append( "\n" );
        ...
        for ( Iterator rowIt = matrix.getRowNames().iterator(); rowIt.hasNext(); ) {
            ...
            if ( rowNameMap != null && rowNameMap.get( rowName ) != null ) rowName = rowNameMap.get( rowName );
            if ( printNames )
