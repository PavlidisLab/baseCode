...
public class MissingFilter extends AbstractFilter  implements Filter {
   ...
   /**
        * Set the maximum fraction of rows which will be removed from the data set. The default value is 0.3
    * Set it to 1.0 to remove this restriction.
    * @param f
    */
   ...
   public DenseDoubleMatrix2DNamed filter(DenseDoubleMatrix2DNamed data) {
      ...
      if (kept < (double) numRows * (1.0 - maxfractionRemoved) && maxfractionRemoved != 0.0) {
         ...
         minpresent = (int) sortedPresent.get( (int) ( (double) numRows * (maxfractionRemoved)));
      }
